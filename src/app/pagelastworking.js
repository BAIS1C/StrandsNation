    ...
    try {
      ...
      if (chainId !== '0xa86a') { // Avalanche C-Chain
        try {
          ...
          // Check if the error is specifically that the chain has not been added to MetaMask.
...
return (
  <div className="min-h-screen relative"> {/* Scanlines and BG from body style */}
    
    <div className="fixed top-6 right-6 z-[9999]"> {/* Adjusted donation button position */}
      <button 
      ...
      >
        <img 
          src="/AVAX-donate.png"
          alt="AVAX Logo" 
          className="w-10 h-10 object-contain opacity-80 group-hover:opacity-100 transition-opacity" /* Smaller: w-10 h-10 */
        />
        <span 
      ...
      {/* Hero Section - Using new body BG, inner container styled with EE subtle dark glass */}
      <section className="w-full px-4 pt-16 pb-20 sm:pt-20 sm:pb-24 md:pt-24 md:pb-28 text-center min-h-[90vh] flex items-center justify-center"> {/* Increased min-h for hero */}
        <div className="bg-[rgba(30,30,45,0.15)] backdrop-blur-[6px] rounded-xl sm:rounded-2xl px-4 py-8 sm:px-6 sm:py-10 md:px-10 md:py-12 shadow-xl w-full max-w-4xl lg:max-w-7xl mx-auto relative overflow-hidden border border-[rgba(255,255,255,0.06)]">
          <div className="absolute -inset-2 sm:-inset-3 bg-[var(--ee-action-taker)]/5 rounded-2xl sm:rounded-3xl blur-xl sm:blur-2xl pointer-events-none animate-pulse-signal opacity-75" /> {/* Softer hero box glow */}
          
          <div className="relative z-10 space-y-5 md:space-y-6">
            
            {/* Logo (top-left of this box) and Title (centered below it or separate) */}
            <div className="relative mb-4 md:mb-6 flex justify-center"> {/* Wrapper for title area */}
                <div className="absolute top-0 left-0 -translate-y-1/4 sm:-translate-y-1/3 md:left-2">
                    <img src="/cards/strandstransplogo64.png" alt="Strands Logo" className="h-12 w-12 sm:h-16 sm:w-16 opacity-70 hover:opacity-90 transition-opacity" />
                </div>
            ...
            <div className="space-y-2 md:space-y-3 max-w-2xl mx-auto">
              ...
              <p className="pulsing-glow-text italic-subtitle text-md sm:text-lg md:text-xl italic"> {/* Removed opacity for class to handle */}
            ...
            <div className="relative w-full max-w-xl lg:max-w-2xl mx-auto pt-4 md:pt-6">
              <div className="p-1 bg-[var(--ee-accent1)]/20 rounded-lg sm:rounded-xl shadow-lg shadow-[var(--ee-accent1)]/20">
                <div className="relative w-full aspect-video rounded-md sm:rounded-lg overflow-hidden border border-[var(--ee-accent1)]/40 hover:border-[var(--ee-accent1)]/70 transition-colors">
                  <iframe
                    src="https://www.youtube.com/embed/XPsQ0X_a2Io?si=5O0FszQcuGjBOsOK" /* IMPORTANT: REPLACE THIS */
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-3 sm:gap-5 pt-5 md:pt-6">
              <button className="btn-glass primary w-full sm:w-auto">
                <span className="btn-text-content">Claim a Passport</span>
              </button>
              <button className="btn-glass secondary w-full sm:w-auto">
                <span className="btn-text-content">Whitepaper</span>
              </button>
              <button className="btn-glass tertiary w-full sm:w-auto">
      ...
      {/* Protocol Cards Section - Responsive Grid */}
      <section className="px-4 sm:px-6 py-10 md:py-12 w-full">
        ...
        <div className="mx-auto max-w-4xl lg:max-w-5xl grid grid-cols-1 sm:grid-cols-3 gap-5 md:gap-6 place-items-stretch"> {/* Changed to sm:grid-cols-3 */}
          {[
...
<footer className="px-6 py-8 md:py-12 text-center bg-[rgba(0,0,0,0.15)] relative mt-8 md:mt-12 border-t border-[rgba(255,255,255,0.05)]">
  ...
  <img
    src="/cards/signal-from-noise.png"
  ...
  <div className="social-icons relative z-10 flex justify-center gap-x-5 sm:gap-x-6">
    <a
  href="https://discord.gg/v7WdD94UXK" // Your existing Discord link
  target="_blank"
...
>
  <svg
    ...
    viewBox="0 0 512 512" // From your SVG
  >
    <g
      transform="translate(0,512) scale(0.1,-0.1)" // From your SVG, may need tweaking
      fill="currentColor" // For CSS color control
    >
      {/* Path 1 (the main body/shape) */}
    ...
    <a href="https://www.youtube.com/@B4SICAI" target="_blank" rel="noopener noreferrer" className="social-icon" aria-label="YouTube">
      {/* Replace your_youtube_channel_link with the one from user "youtube.com/@basicai" */}
      <svg
    className="w-6 h-6" // Standard Tailwind size, adjust as needed (e.g., w-5 h-5)
    viewBox="0 0 512 512" // Taken from your SVG's viewBox attribute
    // Removed width, height, version, xmlns, preserveAspectRatio as they are handled by className or are default
  >
    <g
      transform="translate(0,512) scale(0.1,-0.1)" // Simplified translate/scale, may need tweaking if it looks off
      fill="currentColor" // This is key for CSS color control
      // stroke="none" // Typically not needed if fill="currentColor" is working
    >
    ...
    <a
  href="https://x.com/strandsnation" // Your existing X link
  target="_blank"
  rel="noopener noreferrer"
  className="social-icon"
...
>
  ...
  >
    <g
      transform="translate(0,512) scale(0.1,-0.1)" // From your SVG, crucial for this icon's path data
      fill="currentColor" // For CSS color control
    >
      {/* Path 1 (the outer square and main X body) */}
      ...
      {/* Path 2 (the diagonal stroke part of the X) */}
  ...
  <p className="mt-5 text-xs text-[var(--ee-text-secondary)] relative z-10">© {new Date().getFullYear()} StrandsNation / SomoKasane. All Rights Reserved.</p>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-[rgba(10,10,20,0.8)] backdrop-blur-sm flex items-center justify-center z-[10001] p-4"> {/* Higher z-index for modal */}
          <div className="bg-[var(--ee-anchor)] border border-[var(--ee-accent1)] rounded-lg sm:rounded-xl p-5 sm:p-6 max-w-sm sm:max-w-md w-full relative shadow-2xl shadow-[var(--ee-accent1)]/20">
            ...
            <div className="flex flex-col gap-2.5">
              {modalMessage.includes('Thank you') && (
                <a
                  href="https://discord.gg/S4FehbCjfT" /* IMPORTANT: REPLACE THIS */
                  target="_blank"
                  rel="noopener noreferrer"
