...
const chapter: WhitepaperChapter = {
  ...
  html: `<h1>Whitepaper V6 - Chapter 8: Governance, Privacy, and Compliance</h1>
...
<p>Meaningful personal and contextual data ; including interaction history, preference signals, agent interaction context, consent records, and attention patterns ; lives on device, held within the relevant local persistence and context layers provided by Mymories and EveryWear. Not all categories are stored in the same way or in the same structure, but the principle is uniform: personal data does not transit to centralised servers, does not settle on chain, and remains under user control. Only proofs, attestations, settlement records, provenance, and related verifiable outputs move to the chain where required.</p>
